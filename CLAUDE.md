...
7. `internal/docs/openapi.yaml` (as of 0.6.0) explicitly documents each proxied endpoint per service (tags `forge`/`index`/`weave`/`weave-monitoring`/`content`), with full request/response schemas transcribed from the upstream service's own DTOs/CRD types and an `x-required-permission` (+ `x-resource-type` where applicable) on each path item. When a new upstream endpoint is added, add a matching path item here too — don't just rely on the generic `/api/<svc>/{path}` `pathItems.proxy` catch-all, which is now only a fallback for endpoints not yet itemized. Cross-check the permission you write against the live `rbac.yaml` rather than assuming — trailing wildcards in `route_permissions` can absorb more of the path than expected (e.g. a `DELETE .../artifacts/*` rule also matches `.../artifacts/{id}/types/{typeId}`); the reliable way to check is a temporary test calling `rbac.MatchRoute` (see item 9 below for the `LoadConfig` pattern — same idea, add `MatchRoute` calls for the paths in question, run, delete). Bump `info.version` to match the release either way.
...
**Create-database Job (`postgresql.createDatabaseJob`)**: a separate concern from `db.*` above — `db.*` is the app's own runtime `DB_DSN` (points *at* an already-existing database), while `postgresql.createDatabaseJob` is a Helm `pre-install,pre-upgrade` hook Job (weight `-5`) that creates that database in the first place (idempotent `CREATE DATABASE` against the `postgres` maintenance DB, via `psql`). It does **not** run schema migrations — those stay in the app binary (`internal/db/db.go` `Migrate()`, idempotent `CREATE TABLE IF NOT EXISTS`). Needs its own admin/superuser credentials (`postgresql.external.*`), separate from the `db.*` secret since you can't connect with a DSN naming a database that doesn't exist yet. Secret key defaults to `"password"` (`postgresql.external.existingSecretKey`, overridable) — see the Bitnami note below, this default only matches a hand-rolled secret. Templates: `deployment/templates/postgresql-admin-secret.yaml` (chart-managed secret, skipped when `postgresql.external.existingSecret` is set) + `deployment/templates/postgresql-create-db-job.yaml`. Host is always resolved in the release namespace; only `postgresql.external.host` (and optionally port) needs configuring for a different address.

**RBAC seed Job (`rbacSeed`, `deployment/templates/rbac-seed-job.yaml`)**: a second Helm `pre-install,pre-upgrade` hook Job (weight `-3`, so it runs after `postgresql.createDatabaseJob` but before the main Deployment) that guarantees one group→role admin mapping exists in `group_role_assignments` — otherwise `group_source: db` starts with an empty table and nobody can reach `/bff/admin/*` to create the first assignment (the bootstrap gotcha above). Unlike the create-db job, it connects using the app's own `db.*` `DB_DSN` (not `postgresql.external.*` admin creds) since it only needs INSERT/CREATE TABLE rights on the already-existing app database. Creates the `group_role_assignments` table defensively (`CREATE TABLE IF NOT EXISTS`, kept in sync with `internal/db/db.go`'s schema) so it doesn't depend on the app's own `Migrate()` having run first. The only configurable value is `rbacSeed.adminGroup` (default `platform-admin`) — an OIDC/Keycloak **group** name, not a username: roles are always resolved from the JWT `groups` claim, never a specific user id, so bootstrapping "an admin user" means mapping a group to the `admin` role and making sure a real human is a member of that Keycloak group (out of scope for this chart). **psql gotcha**: `:'var'` substitution (used for the group name) only works reading from a script or heredoc/stdin — verified it silently fails with a syntax error when passed via `psql -c "..."` on `postgres:16-alpine` — hence the job uses a heredoc (`<<'SQL' ... SQL`) instead of `-c`. In a Helm `args: - |` block scalar, all heredoc body lines (including the closing delimiter) must share the exact same base indentation as the rest of the script, since YAML strips only the common leading whitespace — extra/inconsistent indentation on the closing delimiter breaks the heredoc silently (bash won't recognize the terminator and the script hangs/misbehaves).

**Testing a Job template's shell/SQL script before trusting it**: `helm lint`/`helm template` only check that the YAML is well-formed, not that the embedded script actually runs correctly. Extract the rendered `args` string (`helm template ... --show-only templates/<job>.yaml | python3 -c "import yaml,sys; print(yaml.safe_load(sys.stdin)['spec']['template']['spec']['containers'][0]['args'][0])"`), `sh -n` it for a syntax check, then actually execute it against a throwaway `docker run --rm -d -e POSTGRES_PASSWORD=x -e POSTGRES_DB=y -p <port>:5432 postgres:16-alpine` + `docker exec` — run it twice to confirm idempotency. This is what caught both gotchas above.

**Compound value validation in Helm templates**: `required` only checks a single value for emptiness. For a compound condition (e.g. "db.create OR db.existingSecret must be set"), use `{{- if not (or .Values.a .Values.b) }}{{ fail "..." }}{{- end }}` instead — see `postgresql-create-db-job.yaml` / `rbac-seed-job.yaml`.

**Postgres secret conventions differ across sibling services**: fusion-forge hand-rolls its own postgres StatefulSet+Secret with keys `username`/`password`. fusion-index instead depends on the real Bitnami `postgresql` chart (`deployment/Chart.yaml` `dependencies:`, vendored at `deployment/charts/postgresql-*.tgz`) — its Secret uses `postgres-password` for the superuser (has `CREATEDB`) and a *different* key `password` for a separate non-superuser app user. fusion-bff's own dev DB (`.claude.local.md`) actually points at fusion-index's Bitnami-backed instance — when pointing `postgresql.createDatabaseJob` at it, use `username: postgres` + `existingSecretKey: postgres-password`, not the `password` default.
